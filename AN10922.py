...
def bitshift_aes(input):
    ...
    return struct.pack('BBBBBBBBBBBBBBBB', preout[0], preout[1], preout[2], preout[3], preout[4], preout[5], preout[6],
                       preout[7], preout[8], preout[9], preout[10], preout[11], preout[12], preout[13], preout[14],
                       preout[15])
...
def bitshift_des(input):
    ...
    for i in range(0, 8):
        ...
        if i == 7:
            ...
            b = input[i + 1]
            preout.append(((a << 1) | (b >> 7)) % 256)
    return struct.pack('BBBBBBBB', preout[0], preout[1], preout[2], preout[3], preout[4], preout[5], preout[6],
                       preout[7])
...
def restore_version_des(divoutput, version):
    ...
    changed = struct.pack('BBBBBBBB', preout[0], preout[1], preout[2], preout[3], preout[4], preout[5], preout[6],
                          preout[7])
    return changed + divoutput[8:]


def restore_version_des_av2(divoutput, version):
    preout = []
    myversion = version % 256
    for i in range(0, 16):
        unpacked = divoutput[i]
        mybit = (myversion >> (7 - (i % 8))) & 1
        preout.append((unpacked & 254) | mybit)

    changed = struct.pack('BBBBBBBBBBBBBBBB', preout[0], preout[1], preout[2], preout[3], preout[4], preout[5],
                          preout[6], preout[7], preout[8], preout[9], preout[10], preout[11], preout[12], preout[13],
                          preout[14], preout[15])
    return changed
...
def diversify_2tdea_versionrestore_av2(masterkey, divinput, version):
    return restore_version_des_av2(diversify_2tdea_norestore(masterkey, divinput), version)
...
def test():
    ...
    target6 = hexdecode("16f9587d9e8910c96a9748d106117cd7")

    out1 = diversify_aes128(hexdecode("00112233445566778899AABBCCDDEEFF"),
                            hexdecode("04782E21801D803042F54E585020416275"))
    out2 = diversify_aes192(hexdecode("00112233445566778899AABBCCDDEEFF0102030405060708"),
                            hexdecode("04782E21801D803042F54E585020416275"))
    out3 = diversify_2tdea_versionrestore(hexdecode("00112233445566778899AABBCCDDEEFF"),
                                          hexdecode("04782E21801D803042F54E58502041"), 0x55)
    out4 = diversify_3tdea_versionrestore(hexdecode("00112233445566778899AABBCCDDEEFF0102030405060708"),
                                          hexdecode("04782E21801D803042F54E5850"), 0x55)
    out5 = diversify_aes256(hexdecode("00112233445566778899AABBCCDDEEFF0102030405060708090A0B0C0D0E0F00"),
                            hexdecode("04782E21801D803042F54E585020416275"))
    out6 = diversify_2tdea_versionrestore_av2(hexdecode("00112233445566778899AABBCCDDEEFF"),
                                              hexdecode("04782E21801D803042F54E58502041"), 0x55)
    failed = 0
    ...
    if not target6 == out6:
        failed = failed + 1
        print("2TDEA AV2 test failed", hexencode(out6), hexencode(target6))
...
def cli_pgm():
    ...
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        ...
        print("\t<algorithm>, available: aes128, aes192, aes256, 2tdea, 2tdea_av2 (requires version), 3tdea")
        print(
            "\t<masterkey>, hex-string of masterkey, must be 16 byte (aes128,2tdea) or 24 byte (aes192,3tdea) or 32 byte (aes256)")
        print(
            "\t<divinput>, hex-string of diversification input, must be 1..31 (aes128,aes192,aes256) or 1..15 (2tdea,3tdea)")
        print("\t<version>, version number of masterkey, optional, only 2tdea/3tdea")
        print(
            "Diversification input usually consists of PICC UID (7 Byte) + App ID (3 Byte) + ASCII System Identifier (variable length)")
    ...
    if sys.argv[1] == 'aes128':
        ...
        print(
            hexencode(diversify_2tdea_versionrestore(hexdecode(sys.argv[2]), hexdecode(sys.argv[3]), int(sys.argv[4]))))
    elif sys.argv[1] == '2tdea_av2' and len(sys.argv) == 5:
        print(hexencode(
            diversify_2tdea_versionrestore_av2(hexdecode(sys.argv[2]), hexdecode(sys.argv[3]), int(sys.argv[4]))))
